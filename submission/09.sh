...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq '.vout[0].value' | awk '{print $1 * 100000000}')
...
# WRITE YOUR SOLUTION BELOW:
UTXO_TXID=$TXID
UTXO_VOUT_INDEX=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq -r '[.vout[] | select(.value * 100000000 >= 15000000) | .n] | first')
...
# WRITE YOUR SOLUTION BELOW:
TX_SIZE=$((10 + (NUM_INPUTS * 68) + (NUM_OUTPUTS * 31)))
...
# WRITE YOUR SOLUTION BELOW:
VERIFY_RBF=$(echo "$DECODED_TX" | jq -r '.vin | map(.sequence < 4294967295) | any')
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(bitcoin-cli -regtest decoderawtransaction "$RAW_TX" | jq -r '[.vout[] | select(.value * 100000000 >= 10000) | .n] | first')
...
NUM_INPUTS=$(echo "$CHILD_INPUTS" | jq 'length')
NUM_OUTPUTS=$(bitcoin-cli -regtest decoderawtransaction "$RAW_TX" | jq '.vout | length')


# STUDENT TASK: Calculate fees, allowing for a high fee to help the parent transaction
CHILD_TX_SIZE=$(( (NUM_INPUTS * 148) + (NUM_OUTPUTS * 34) + 10 ))
...
PARENT_TX_AMOUNT=$(bitcoin-cli -regtest gettxout "$PARENT_TXID" "$CHANGE_OUTPUT_INDEX" | jq -r '.value')


# Calculate the amount to send after deducting fee
CHILD_RECIPIENT="2MvM2nZjueT9qQJgZh7LBPoudS554B6arQc"
CHILD_SEND_AMOUNT=$(echo "$PARENT_TX_AMOUNT - ($CHILD_FEE_SATS / 100000000)" | bc)
