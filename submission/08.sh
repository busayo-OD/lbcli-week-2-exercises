# Create a transaction whose fee can be later updated to a higher fee if it is stuck or doesn't get mined on time

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# raw_tx=""


#!/bin/bash
# This script creates an RBF-enabled raw transaction that spends all UTXOs
# from the provided raw transaction to send 0.2 BTC to the recipient.
# The fee is the difference between total input and sent amount.

BITCOIN_CLI="bitcoin-cli -regtest"

# Provided raw transaction
raw_tx=""

# Step 1: Decode raw transaction
DECODED=$($BITCOIN_CLI decoderawtransaction "$raw_tx")

# Extract parent transaction ID
PARENT_TXID=$(echo "$DECODED" | jq -r '.txid')

# Step 2: Extract UTXO values
VALUE0=$(echo "$DECODED" | jq -r '.vout[0].value')
VALUE1=$(echo "$DECODED" | jq -r '.vout[1].value')
TOTAL_INPUT=$(echo "$VALUE0 + $VALUE1" | bc -l)

# Step 3: Define the amount to send and calculate fee
SEND_AMOUNT=0.20000000   # 20,000,000 satoshis = 0.2 BTC
FEE=$(echo "scale=8; $TOTAL_INPUT - $SEND_AMOUNT" | bc)

# Step 4: Construct inputs with RBF enabled (nSequence < 0xffffffff)
INPUTS=$( jq -n --arg txid "$PARENT_TXID" '[
  {"txid": $txid, "vout": 0, "sequence": 4294967293},
  {"txid": $txid, "vout": 1, "sequence": 4294967293}
]' )

# Step 5: Construct outputs (sending 0.2 BTC to recipient)
RECEIVER_ADDR="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
OUTPUTS=$( jq -n --arg addr "$RECEIVER_ADDR" --argjson amt "$SEND_AMOUNT" '{($addr): $amt}' )

# Step 6: Create raw transaction
NEW_RAW_TX=$($BITCOIN_CLI createrawtransaction "$INPUTS" "$OUTPUTS")

# Output the new raw transaction
echo "Raw Transaction (RBF Enabled):"
echo "$NEW_RAW_TX"
